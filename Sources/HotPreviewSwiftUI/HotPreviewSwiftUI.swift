//
//  HotPreviewSwiftUI.swift
//  testSwiftUI
//
//  Created by Sang Truong on 16/09/2023.
//
...
struct PreviewResizableViewModifier: ViewModifier {
    @StateObject private var state = CustomBorderState()
        ...
        VStack {
            if hiddenToolbar {
                VStack(alignment: .trailing,spacing: 12) {
                    Button {state.debugBordersEnabled1.toggle()} label: {Text("Border = \(state.debugBordersEnabled1 ? "enable" : "disable")")}
                        .tint(state.debugBordersEnabled1 ? .green : .red)
...
class CustomBorderState : ObservableObject{
    @Published private(set) var listSize : [BorderManager] = GLOBAL_BORDER_TRACKERS
    var debugBordersEnabled1 = false {
       didSet {
           UIView.swizzleDebugBehaviour_UNTRACKABLE_TOGGLE()
           guard debugBordersEnabled1 else {
...
class BorderManager {
    ...
    let outlineColor: CGColor = Color.random.cgColor!
